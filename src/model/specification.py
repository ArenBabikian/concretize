import copy
from scenic.domains.driving.roads import Network

from src.model.actor import Actor

class Specification:
    def __init__(self, actors, constraints):
        # self.map_file = None
        # self.roadmap = None
        self.actors = actors
        self.constraints = constraints
        # self.tested_junction = None

    # def __init__(self, map_file):
        # self.map_file = map_file
        # self.roadmap = self.parsemap(map_file)
        # self.actors = None
        # self.constraints = None
        # self.tested_junction = None

    def parsemap(self, map_file):
        # TODO leverage Scenic
        return Network.fromFile(map_file)

    def __str__(self):
        return f"""
        Map: {self.map_file}
        Actors: {self.actors}
        Constraints: {self.constraints}
        """


class Specification_Instance:
    def __init__(self, specification):
        self.specification = specification
        # BIG TODO: integrate some kind of traceability to avoid copying everything
        # TODO: deepcopy has some trouble with models generated by textX somehow
        # self.actors = copy.deepcopy(specification.actors)
        self.actors = []
        for actor in specification.actors:
            newActor = actor.__new__(actor.__class__)
            for key, value in actor.__dict__.items():
                setattr(newActor, key, value)
            self.actors.append(newActor)
        # self.constraints = copy.deepcopy(specification.constraints) # will still be connected to meta-actors.
        self.constraints = []
        for constraint in specification.constraints:
            newConstraint = constraint.__new__(constraint.__class__)
            for key, value in constraint.__dict__.items():
                setattr(newConstraint, key, value)
            self.constraints.append(newConstraint)
        self.relink_consraints_to_actors()
        self.roadmap = specification.roadmap

        # post-run gathered data
        self.raw_res = None
        self.n_violations = None
        self.constraint2heuristic = None

    # TODO below is pretty ugly, tbh
    def relink_consraints_to_actors(self):
        for concrete_constraint in self.constraints:
            concrete_actor_list = []
            for meta_actor in concrete_constraint.actors:
                if not isinstance(meta_actor, Actor):
                    concrete_actor_list.append(meta_actor)
                else:
                    for concrete_actor in self.actors:
                        if concrete_actor.id == meta_actor.id:
                            concrete_actor_list.append(concrete_actor)
            concrete_constraint.actors = concrete_actor_list
